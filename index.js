...
| $$  | $$| $$  | $$  | $$ /$$| $$ /$$| $$_____/| $$      
| $$  | $$|  $$$$$$/  |  $$$$/|  $$$$/|  $$$$$$$| $$      
|__/  |__/ \______/    \___/   \___/   \_______/|__/      v1.1
...
process.title = `Loading Generator`

// Console logs
var repeat = 1
if (repeat < 2) {
  const o = fs.createWriteStream('./LOGS.log', {flags:'a'})
  const errorOutput = fs.createWriteStream('./LOGS.log', {flags:'a'})
  ...
  function write_log(goodnews, text){
    ...
    console.log(`${color} [BOOST] \x1b[37m ${text}`);
  ...
  async function break_captcha(DiscordPage){
    try {
      ...
      write_log(false, "Received Captcha");
      ...
      while(true){
        try{
          ...
          write_log(true, "Bypassed Captcha");
          return true;
        } catch(err) {
          write_log(false, "Failed To Bypass Captcha");
          sleep(3000);
        }
      }
    } catch(e){
      write_log(true, "Can Not Find Captcha");
    };
  }
  ...
  async function create_accinfos(browser, d) {
    ...
    client.on('Network.webSocketFrameSent', ({requestId, timestamp, response}) => {
      try {
        ...
        if(!token && json["d"]["token"]){
          token = json["d"]["token"];
          write_log(true, `Generated Token: ${token}`);
        };
      } catch(e){};
    })
    ...
    write_log(true, "Email Verified Account");
